...
- `_validate_configured_base_urls` runs once at startup and surfaces any operator-configured base URL (env or DB setting: `OPENAI_BASE_URL`, `WHISPER_API_BASE_URL`, `OPENROUTER_BASE_URL`, `ANTHROPIC_BASE_URL`, `openai_base_url`, `whisper_api_base_url`) that would fail SSRF validation. The error is logged at `ERROR` level but does not abort startup; the fetch-time validators still refuse the URL if it is actually used.
- Reverse-proxy awareness: when `MINUSPOD_TRUSTED_PROXY_COUNT` is set to `N >= 1`, Werkzeug's `ProxyFix` middleware reads the client IP from the last `N` entries of `X-Forwarded-For`. Login lockout decisions key on `request.remote_addr`; without ProxyFix a deployment behind Cloudflare (or cloudflared) sees every failed login as coming from the proxy and lockout never fires. A container without the env var set now logs a startup WARN so the regression is discoverable.
