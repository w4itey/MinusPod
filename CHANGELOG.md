...
- `/docs` and `/openapi.yaml` moved under `/api/v1/`. The legacy root paths 301-redirect so existing browser bookmarks and Portainer health checks keep working. Swagger UI now loads `swagger-ui-dist` assets from `/ui/swagger/*` (bundled in the Docker image at build time) instead of the `unpkg.com` CDN. CSP and air-gapped deployments no longer require an external exception.
