...
- `RSSParser.fetch_feed`, `GET /api/v1/podcast-search`, `pricing_fetcher.fetch_openrouter_pricing`, `pricing_fetcher.fetch_pricepertoken_pricing`, `storage.download_artwork`, and `webhook_service._prepare_and_dispatch` all route through `utils.safe_http` (`safe_get` / `safe_post`) so every redirect hop is revalidated against the SSRF rules and HTTPS -> HTTP downgrades are blocked per-hop.
